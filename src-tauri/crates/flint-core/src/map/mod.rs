...
use crate::wad::read_chunk_decompressed_bytes;
...
fn extract_one(
    ...
    let data = read_chunk_decompressed_bytes(wad_path, &chunk).map_err(|e| Error::Wad {
...
pub fn extract_variant_files(
    ...
    let by_hash: HashMap<u64, WadChunk> = toc
        .chunks.iter()
        .map(|c| (c.path_hash, *c))
        .collect();
